...
if (config.LANG == 'EN') {
    QueenAmdiCon.on('QueenAmdiConecting', async () => {
        console.log(`${chalk.green.bold('Queen')}${chalk.blue.bold('Amdi')}
    ${chalk.white.bold('Version:')} ${chalk.red.bold(config.VERSION)}

    ${chalk.blue.italic('ℹ️ QueenAmdiConecting to WhatsApp... Please wait...')}`);
        });
}
else if (config.LANG == 'SI') {
    QueenAmdiCon.on('QueenAmdiConecting', async () => {
...
if (config.WORKTYPE == 'public') {
    ...
    if (QueenAmdiCon.user.jid === '@s.whatsapp.net') {
        ...
        await QueenAmdiCon.sendMessage(QueenAmdiCon.user.jid, '*Nexa started in Private Mode👸*\n\n_Please do not try any commands here. This is your log number._\n_You can try commands anywhere else :)_\n\n_Type_ *.panel* _to get your full command list._\n\n_Your bot in Private Mode. To change, use_ ```.setvar WORK_TYPE=public``` _command._\n\n*You can check our website for new features : https://www.stahseen.me/*\n\n*Contact here to request new features:*\nhttps://wa.me/923018706705\n\n*Updates:* https://gist.github.com/BlackAmda/890b6b31fcb8d376d6a68afcb7359324\n\n *Thank you for using Nexa 💌*', MessageType.text);
    }
}
    else {
                ...
                if ((command.on !== undefined && (command.on === 'image' || command.on === 'photo')
                    ...
                    if (sendMsg) {
                        ...
                        try {
                            ...
                            if (config.LANG == 'SI') {
                                await QueenAmdiCon.sendMessage(QueenAmdiCon.user.jid, '*-- දෝෂ වාර්තාව [Nexa] --*' + 
                                    ...
                                    '\n_Support Group 01:_ https://chat.whatsapp.com/Hkm79J2sapyLGD0rvRTfkq' +
                                    '\n_Support Group 02:_ https://chat.whatsapp.com/LN48JV7pYGS6NGwNDeiTUF' +
                                    '\n\n_මෙම පණිවිඩය ඔබගේ අංකයට ගොස් තිබිය යුතුය (සුරකින ලද පණිවිඩ)._\n\n' +
                                    '*සිදු වූ දෝෂය:* ```' + error + '```\n\n'
                                    , MessageType.text, {detectLinks: false}
                                );
                                if (error.message.includes('URL')) {
                                    return await QueenAmdiCon.sendMessage(QueenAmdiCon.user.jid, '*⚜️ දෝෂ විශ්ලේෂණය [Nexa] ⚜️*' + 
                                        '\n==== ```දෝෂ නිරාකරණය කර ඇත!``` ====' +
                                ...
                                await QueenAmdiCon.sendMessage(QueenAmdiCon.user.jid, '*-- ERROR REPORT [NEXA] --*' + 
                                    '\n\n*Nexa an error has occurred!*'+
                                    '\n\n_This error log may include your number or the number of an opponent. Please be careful with it!_' +
                                    '\n\n_Aslo you can join our support group:_' +
                                    '\n_Support Contact 01:_ https://wa.me/923018706705' +
                                    '\n\n_This message should have gone to your number (saved messages)._\n\n' +
                                    '*Error:* ```' + error + '```\n\n'
                                    , MessageType.text, {detectLinks: false}
                                );
                                if (error.message.includes('URL')) {
                                    return await QueenAmdiCon.sendMessage(QueenAmdiCon.user.jid, '*⚜️ ERROR ANALYSIS [NEXA] ⚜️*' + 
                                        '\n========== ```Error Resolved!``` ==========' +
